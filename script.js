...
function setFilterRenewalDate(client, filterIndex, active) {
  ...
  let nextDueDate = client.renewalDates[filterIndex]
    ? parseDate(client.renewalDates[filterIndex])
    : addMonths(getClientBaseDate(client), interval);
...
function bindFilterButtons(client) {
  document.querySelectorAll(".toggle-filter-btn").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      const isActive = Boolean(targetClient.renewalDates?.[filterIndex]);
      setFilterRenewalDate(targetClient, filterIndex, !isActive);
      saveClients(clients);
      renderDetailsPage();
      updateHomeReminderBanner();
    });
